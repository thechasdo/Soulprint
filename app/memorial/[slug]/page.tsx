...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!profile || profile.visibility === "private") {
    return {
      title: "Soulprint Memorial",
      description: "A secure family archive where memories live on.",
    };
  }
  ...
  return {
    ...
    description: profile.headline || `In loving memory of ${profile.full_name}. Explore their stories, biography, timeline, and family memories on Soulprint.`,
    openGraph: {
      title: `${profile.full_name}${lifeSpan} — Soulprint`,
      ...
      images: [
        {
          ...
          width: 1200,
          height: 1200,
          alt: `${profile.full_name} Memorial on Soulprint`,
        }
      ],
    },
    twitter: {
      ...
      <div className="mx-auto max-w-4xl">
        <div className="flex justify-between items-center">
          ...
          <span className="flex items-center gap-1.5 rounded-full bg-white border border-navy/10 px-4 py-1.5 text-xs font-black uppercase tracking-wider text-navy/70">
            <Globe className="h-3.5 w-3.5 text-seafoam" /> {profile.visibility} Memorial
          </span>
        </div>
