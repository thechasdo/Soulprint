...
export default function ProfilesPage() {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || "An error occurred while saving Soulprint Profile.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDeleteProfile = async (id: string) => {
    if (!confirm("Are you sure you want to delete this Soulprint Profile? All timeline events and tributes will be permanently deleted.")) return;
  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-12">
      <div className="flex items-center justify-between">
        ...
        >
          <Plus className="h-4 w-4" /> Add Soulprint Profile
        </button>
      </div>

      <h1 className="mt-8 text-4xl font-black text-navy">Soulprint Profiles</h1>
      <p className="mt-3 max-w-2xl leading-8 text-navy/70">
        Create internal Soulprint Profiles for loved ones, family history, and family timeline branches. Each public-facing version becomes a Soulprint with relationships, Ancestry links, stories, and uploaded assets.
      </p>
      ...
      {loading ? (
        ...
        <div className="mt-8 rounded-brand border border-dashed border-navy/20 bg-white p-12 text-center">
          <p className="font-black text-navy text-xl">No Soulprint Profiles created yet</p>
          <p className="mt-2 text-sm text-navy/60 max-w-md mx-auto">
            Get started by creating your first Soulprint Profile. You can add photos, timelines, and biographical links once the profile is created.
          </p>
          <button
            onClick={handleOpenCreateModal}
          ...
          >
            <Plus className="h-4 w-4" /> Create First Soulprint Profile
          </button>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy/40 backdrop-blur-sm p-5 overflow-y-auto">
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl p-8 shadow-glow max-h-[90vh] overflow-y-auto">
            <h2 className="text-3xl font-black text-navy">
              {editingProfile ? "Edit Soulprint Profile" : "Create New Soulprint Profile"}
            </h2>
            <form onSubmit={handleSaveProfile} className="mt-6 grid gap-6">
              ...
              <div className="grid gap-2">
                ...
                >
                  ...
                  <option value="public">Public (Indexed and searchable on the public Soulprint directory)</option>
                </select>
              </div>
